      ...
      <Card>
        <CardBody className="space-y-4">
          <div className="flex items-center justify-between text-sm">
            <span className="text-charcoal/60">{t("invoice")}</span>
            <span className="font-mono text-charcoal">{order.invoiceNo}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="font-medium text-charcoal">{order.planName}</span>
          ...
          <dl className="space-y-1.5 text-sm">
            <div className="flex justify-between">
              ...
              <dd className="text-charcoal">৳{formatTaka(order.baseAmount)}</dd>
            </div>
            {order.discountAmount > 0 && (
              <div className="flex justify-between text-trustGreen">
                <dt>
                  {t("discount")}
            ...
            <div className="flex justify-between border-t border-charcoal/10 pt-2 text-base font-semibold">
              ...
              <dd className="text-charcoal">৳{formatTaka(order.finalAmount)}</dd>
            </div>
          </dl>
          ...
          {order.status === "PAID" ? (
            <Link
              href={successHref}
              className="block rounded-xl bg-trustGreen px-4 py-3 text-center text-sm font-medium text-white"
            >
              {t("alreadyPaid")}
            </Link>
          ) : (
