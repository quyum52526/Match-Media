...
import { createAgencyClientProfile } from "@/lib/actions/mediaClients";
import {
  GENDERS,
  PROFESSIONS,
  EDUCATION_LEVELS,
...
export function AddClientModal({ open, onClose }: Props) {
  const t = useTranslations("MediaDashboard.addModal");
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await createAgencyClientProfile({
        fullName,
        gender,
        dateOfBirth,
    ...
    <Modal open={open} onClose={handleClose} title={t("title")}>
      ...
      {success ? (
        <div className="flex flex-col items-center gap-3 py-6 text-center">
          <span className="flex h-12 w-12 items-center justify-center rounded-full bg-success/10 text-success text-2xl">
            ✓
          </span>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className={LABEL}>
                {t("dateOfBirth")} <span className="text-primary">*</span>
              </label>
              <input
                type="date"
                value={dateOfBirth}
